...
public class QuizLauncher extends QuizActivity {
  ...
  /**
   * The setAnswerClickListeners function sets the onClickListener for each answer TextView. When an
   * answer is clicked, it checks if the text of that TextView matches the correctAnswer string in
   * its corresponding Question object. If it does match, then we increment our correct variable by
   * 1 and set a green background color to that TextView. Otherwise, we set a red background color
   * to that TextView and change its textColor to white so it's visible against the red background.
   */
    ...
    for (TextView answer : answerTextViews) {
      answer.setOnClickListener(
          v -> {
            ...
            if (questionRef
                .getCorrectAnswer(questionRef.getAnswers())
                .contentEquals(answer.getText())) {
              correct++;
  ...
  /**
   * The moveToNextQuestion function is called when the user clicks on a button. It checks if there
   * are more questions to be answered, and if so, it sets the next question screen. If not, it
   * updates the score and starts an intent to go to ResultActivity.java
   *
  ...
  /**
   * The startCountDownTimer function is used to start the countdown timer for each question. The
   * function takes in an integer parameter, which represents the index of the current question. The
   * function then sets up a CountDownTimerView object and starts it with a duration of 60 seconds
   * (60000 milliseconds). When this timer finishes, we want to move on to the next question by
   * calling our moveToNextQuestion method.
   *
  ...
  void startCountDownTimer(final int questionIndex) {
    ...
    countdownTimerView.setOnTimerFinishedListener(
        () -> runOnUiThread(() -> moveToNextQuestion(questionIndex)));
  }
}
