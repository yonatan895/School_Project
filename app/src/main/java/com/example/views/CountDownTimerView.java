...
public class CountDownTimerView extends View {
  ...
  /**
   * The init function initializes the textPaint object, which is used to draw the text on the
   * canvas. It sets its color to black and its size to 40 pixels.
   */
  private void init() {
    textPaint = new Paint();
  ...
  /**
   * The onDraw function is called whenever the view needs to be redrawn. This happens when the view
   * is first displayed, and then again every time it's invalidated. The Canvas parameter passed in
   * represents a drawing surface that you can use to draw on.
   *
