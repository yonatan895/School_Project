...
public abstract class QuizActivity extends AppCompatActivity {
  ...
  /**
   * The updateScore function updates the user's score in Firebase. It does this by first getting a
   * reference to the database and then getting a reference to the current user. If there is no
   * current user, it will not update anything. Otherwise, it will get a document reference for that
   * specific user from FirebaseFirestore db and then check if their answer was correct or incorrect
   * using an if statement. If their answer was correct, it increments totalCorrectAnswers by 1 and
   * totalQuestions by 1 (since they answered one question). Otherwise, only totalQuestions is
   * incremented since they did not get that question right. Then we
   *
